...
fn polygon_location(device: &wgpu::Device, sides: u32, location: Point) -> Mesh {
    let mut geometry = Geometry::new();

    for i in 0..sides {
        let angle = (PI / 2.0) + (i as f32 / sides as f32) * 2.0 * PI;

        let x = angle.cos() + location.x;
        let y = angle.sin() + location.y;
...
fn get_meshes(device: &wgpu::Device) -> Vec<Mesh> {
    vec![
        polygon_location(&device, 6, Point::new(3.0, 2.0, 0.0))
    ]
}
...
impl UnitIdentifierTrait for PolygonLocation {
    fn new() -> Self {
        Self {}
    }

    fn identifier(&self) -> String {
        String::from("polygon_location")
    }

    fn label(&self) -> String {
        String::from("Lesson 2: Polygon Location")
    }
}

impl CanvasUnitTrait for PolygonLocation {
...
impl UnitTrait for PolygonLocation {}